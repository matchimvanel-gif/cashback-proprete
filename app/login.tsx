...
export default function ecranLogin() {
  ...
  async function reinitialiserMotDePasse() {
    ...
    try {
      ...
      if (e.code === "auth/user-not-found")
        afficherErreurTemporaire("Aucun compte avec cet email.");
      if (e.code === "auth/invalid-email")
        afficherErreurTemporaire("Email invalide.");
    }
  }
  ...
  async function seConnecter() {
    ...
    demarrerProgression();
    try {
      //try catch est l'equivalent de if else juste qu'on essaie try et si sa marche pas on vas dans catch
      ...
      // e contient le detail de l'erreur
      if (e.code === "auth/invalid-credential")
        afficherErreurTemporaire("Email ou mot de passe incorrect.");
      if (e.code === "auth/too-many-requests")
        afficherErreurTemporaire("Trop de tentatives. Réessaie plus tard.");
      if (e.code === "auth/network-request-failed")
        afficherErreurTemporaire("Pas de connexion internet.");
      if (e.code === "auth/email-already-in-use")
        afficherErreurTemporaire("Cet email est déjà utilise.");
      if (e.code === "auth/weak-password")
        afficherErreurTemporaire(
      ...
      if (e.code === "auth/too-many-requests")
        afficherErreurTemporaire("Trop de tentatives. Réessaie plus tard.");
      if (e.code === "auth/invalid-email")
        afficherErreurTemporaire("entrer une addresse email valide");
      if (e.code === "auth/email-already-in-use")
        afficherErreurTemporaire("Cet email est déjà utilise.");
      if (e.code === "auth/invalid-email")
        afficherErreurTemporaire("entrer une adresse email valide");
      else {
    ...
    if (chargement === true) {
      return (
        <View style={{ width: "100%", alignItems: "center" }}>
          <Text
            style={{ color: couleur.marine, opacity: 0.7, marginBottom: 8 }}
          >
