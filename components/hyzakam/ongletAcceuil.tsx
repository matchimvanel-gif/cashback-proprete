...
export default function OngletAccueil() {
  ...
  const carteStats = [
    {
      ...
      valeur: statsJour.depots.toString(),
      icone: "trash-outline",
    },
    {
      label: "Coupons validés",
      valeur: statsJour.coupons.toString(),
      icone: "pricetag-outline",
    },
    {
      label: "Partenaires actifs",
      valeur: statsJour.partenaires.toString(),
      icone: "storefront-outline",
    },
    {
      label: "Points distribués",
      valeur: statsJour.points.toString(),
      icone: "star-outline",
    },
  ];
  ...
  async function chargerDonnees() {
    ...
    try {
      ...
      const snapshotPartenaires = await getDocs(
      ...
      const alertesFirebase = snapshotAlertes.docs.map(function (document) {
        ...
        const multiplicateur = lireNombre(
          data.multiplicateur ?? data.valeur,
          0,
        );
  ...
  async function ajouterVille() {
    ...
    await updateDoc(doc(db, "configuration", "zones"), {
      villes: nouvelleListe,
    });
  ...
  async function ajouterQuartier() {
    ...
    const nouvelleListe = [
      ...tousQuartiers,
      { nom: nouveauQuartier.trim(), ville: villeChoisie },
    ];
    await updateDoc(doc(db, "configuration", "zones"), {
      quartiers: nouvelleListe,
    });
  ...
  return (
    <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 16 }}>
      <Text style={stylesTitre.titre}>Tableau de bord</Text>

      <Text style={stylesTitre.sousTitre}>Aujourd'hui</Text>
      ...
      <Text style={stylesTitre.sousTitre}>⚠️ Alertes responsables</Text>
      ...
      <Text style={stylesTitre.sousTitre}>🏘️ Noter un quartier</Text>
      ...
      {villeChoisie !== "" && (
        <>
          ...
          >
            <Text
              style={{ color: quartierChoisi === "" ? "#ccc" : couleur.blanc }}
            >
              {quartierChoisi === ""
                ? "Choisir un quartier..."
                : quartierChoisi}
              ...
              {quartiersFiltres.map((q) => (
                <TouchableOpacity
                  ...
                  style={{
                    padding: 14,
                    borderBottomWidth:
                      q === quartiersFiltres[quartiersFiltres.length - 1]
                        ? 0
                        : 0.5,
                    borderBottomColor: "rgba(255,255,255,0.2)",
                  }}
          ...
          {quartierChoisi !== "" && (
            <View style={{ flexDirection: "row", gap: 10, marginBottom: 30 }}>
              <TouchableOpacity
                style={{
                  flex: 1,
                  backgroundColor: couleur.vert,
                  borderRadius: 15,
                  padding: 14,
                  alignItems: "center",
                  borderColor: couleur.blanc,
                  borderWidth: 0.5,
                }}
              ...
              >
                ...
                <Text
                  style={{ color: "#fff", marginTop: 5, fontWeight: "bold" }}
                >
                  Propre
                </Text>
              ...
              <TouchableOpacity
                style={{
                  flex: 1,
                  backgroundColor: "rgba(94,41,35,0.7)",
                  borderRadius: 15,
                  padding: 14,
                  alignItems: "center",
                  borderWidth: 1,
                  borderColor: couleur.erreurClair,
                }}
      ...
      <Text style={stylesTitre.sousTitre}>🌍 Gérer les zones</Text>

      <Text
        style={{ color: couleur.dore, marginBottom: 8, fontWeight: "bold" }}
      >
        Villes
      </Text>
      {villes.map((ville) => (
