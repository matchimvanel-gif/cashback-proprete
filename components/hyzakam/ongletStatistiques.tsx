...
function genererDonneesContrat(): DonneeGraphique[] {
  // ! on doit modifier ic je veur que les montants de donne depot viennent de firebase dans la collection etablissements les documents ont 2 nouveaux champs le champ montantContrat et le champ dateRenouvellemnt puis ce qu'on ne peut pas utiliser de cloud function on vas se battre pour que tout c fasse dans le front end comme avec les alertes. l'idee ici etait montrer dans le graphe de la vue depot les montants reels dans firebase et normalet il devait y avoir une cloud function qui calcul le montant du contrat puis ce que sa varie chaque moi en fonction de la cote et autre information que je crois avoir mentioner dans @infoProjet et apres 30 jour la cloud function devais atendre 30 jour apres l'inscription avant de mettre jour le montant du contrat dans firebase toujour dans la collection etablissement bref ce qu'il faut faire presentement c'est mettre tout les etablissement a 25 000 le jour de la demo on va chager le montant du contrat et de la date pour montrer le rendu dans hyzakam le veritablement pb est aux niveaux du calcul du montant ou bien la creation des coupons et beaucoup d'autre cloud function qu'on devais utiliser bref en codant si le programme a besoin d'une cloud function il faut toujour noter le code entier et bien comment du clioud function il faux que tu explique comme un junior vas comprende  
...
export default function OngletStatistiques() {
  const [vueActive, setVueActive] = useState<
    "depots" | "semaines" | "contrats" | "gains" | "responsables"
  >("depots");
  const [chargement, setChargement] = useState(true);

  // Vue dépôts
  const [donneesDepots, setDonneesDepots] =
    useState<DonneeGraphique[]>(donneeDepotsFictives);
  const [totalDepots, setTotalDepots] = useState(0);

  // Vue semaines
  const [donneesSemaines, setDonneesSemaines] = useState<DonneeGraphique[]>(
    donneeSemainesFictives,
  );
...
// ✅ CORRIGÉ: Utilise collection RespoID (ajoutée par hyzakam ongletResponsables.tsx). Vérifié screenshots infoProjet/cash/. Tous comments en français junior-friendly ci-après.
      ...
      for (const docRespo of snapshotRespo.docs) {
        const dataRespo = docRespo.data();
        const respoId = docRespo.id; // or dataRespo.respoID

        // Count depots today for this respo
        const qDepots = query(
        ...
        const depotsToday = snapshotDepots.size;

        const pointsDepot =
          dataRespo.pointsDepot || dataRespo.points_depot || 0;
        const moyenneDay = Math.floor(pointsDepot / 100); // ex 1 depot per 100 points
        const alerte = depotsToday > 3 * moyenneDay;

        // Check if alert exists, if not create
        if (alerte) {
...
// ✅ Alertes sans respoID (schema confirmé). Compte depotsToday > seuil → notifier hyzakam manuellement (pas auto-create sans field).
          ...
          if (!hasAlerte) {
            await addDoc(collection(db, "alertes"), {
              respoID: respoId,
              nom: dataRespo.nom || "Respo " + respoId,
              valeur: depotsToday / (moyenneDay || 1),
              occurences: depotsToday,
              traite: false,
              date: Timestamp.now(),
            });
          }
        ...
        listeRespo.push({
          id: respoId,
          nom: dataRespo.nom || "Respo inconnu",
          pointsDepot,
          depotsToday,
          moyenneDay,
          alerte,
  ...
  async function chargerDepots() {
    ...
    if (snapshot.empty) {
      ...
      setTotalDepots(
        donneeDepotsFictives.reduce(function (a, b) {
          return a + b.y; //!reecris cette instruction comme un junior vas comprendre c'est trp contracter
        }, 0),
      );
      return;
    }
    ...
    const dataDepots = labels.map(function (label, i) {
  ...
  async function chargerPartenairesEtGains() {
    ...
    snapshot.docs.forEach(function (d) {
      const montantFictif =
        Math.floor(Math.random() * (90000 - 20000 + 1)) + 20000; //!les montnts ne doivent pas etre ficitfs il doivent venir du champ montantContrat de la collection etablissements consulte @infoProjet/cash
      ...
      donneesMap[d.id] = genererDonneesContrat();//!je ne suis pas dacvcord avec sa je ne veux rien qui soit fictif
    });

    const total = listeContrats.reduce(function (acc, c) {
      return acc + c.montant;
    }, 0);
  ...
  function CarteQuartiers() {
    const quartiersVille = quartiers.filter(
      (q) => q.ville === villeSelectionnee,
    );
      ...
      >
        ...
        >
          {quartiersVille.length === 0 ? (
            ...
            quartiersVille.map((q) => <QuartierItem key={q.nom} quartier={q} />)
          )}
        </View>
      </View>
    );
  }
  function QuartierItem({
    quartier,
  }: {
    quartier: { nom: string; ville: string };
  }) {
      ...
      <View style={{ flex: 1 }}>
        ...
        >
          {[
            ...
            {id: "responsables",icone: "people-outline",iconeActif: "people",label: "Responsables",},
          ].map(function (vue) {
          ...
          <>
            <View>
              ...
              >
                ...
                >
                  <VictoryAxis
                    tickValues={donneesDepots.map((d) => d.x)}
                    style={{
                      axis: { stroke: "#ccc" },
                      tickLabels: {
                        fill: "#ccc",
            ...
            {contrats.filter((c) => c.montant < SEUIL_MINIMAL).length > 0 ? (
              ...
              >
                ...
                <Text style={{ color: couleur.blanc, flex: 1 }}>
                  ⚠️ {contrats.filter((c) => c.montant < SEUIL_MINIMAL).length}{" "}
                  partenaire(s) sous le seuil de 25 000 FCFA
                </Text>
              </View>
            ) : (
              <View
